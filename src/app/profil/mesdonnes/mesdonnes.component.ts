...
@Component({
  selector: 'app-mesdonnes',
  templateUrl: './mesdonnes.component.html',
  styleUrls: ['./mesdonnes.component.css']
})
export class MesdonnesComponent implements OnInit {
  ...
  editProfil(){   //console.log("gooooooooooo")
    ...
    if(this.user.role=="ADMIN"){
      data = Object.assign(data, {email:this.user.email,password:this.user.password, idUser:this.user.idUser, role:"ADMIN"})
      this.saveAdmin(data);
      return
    }else if(this.user.role=="MEMBER1"){
      data = Object.assign(data, {email:this.user.email,password:this.user.password, idUser:this.user.idUser, role:"APPRENANT"})
