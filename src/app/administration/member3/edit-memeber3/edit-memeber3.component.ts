...
@Component({
  selector: 'app-edit-memeber3',
  templateUrl: './edit-memeber3.component.html',
  styleUrls: ['./edit-memeber3.component.css']
})
export class EditMemeber3Component implements OnInit {
  addForm!: FormGroup; 
  idApp!:number; 
  app:any={};
  ...
  editMember1(){ 
    let data = {idUser: this.idApp, lastName: this.addForm.value.lastName ,firstName: this.addForm.value.firstName,
      ...
      tel: this.addForm.value.tel, email: this.addForm.value.email ,password: this.addForm.value.password ,role:"MEMBER3"} ;
    ...
    this.member2Service.edit(data).subscribe({
      next: (res) => {
        if(res.status=="success"){
         ...
         this.router.navigate(['admin/memeber3/']); 
        }else{
